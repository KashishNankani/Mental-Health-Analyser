...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ------------------ SIGNUP (only if present on page) ------------------ */
  if (signupForm) {
  ...
  onAuthStateChanged(auth, (user) => {
    ...
    if (!user && protectedPages.includes(currentPage)) {
      ...
      window.location.href = "login.html"; // <- uses login.html (your file)
      return;
    }

    // Optional nicety: if user is already logged in and opens login.html, send them to form1
